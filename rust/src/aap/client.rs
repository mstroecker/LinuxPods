...
//!
//! Port of internal/aap/client.go. The Go version hand-rolled an AF_BLUETOOTH
//! SOCK_SEQPACKET socket with a manually laid out `sockaddr_l2` and raw
//! `syscall.Syscall(SYS_CONNECT, ...)`; bluer provides the same socket as a safe,
//! typed, async API, so the `unsafe` block is gone entirely.
//!
//! Protocol flow:
